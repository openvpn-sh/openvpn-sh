...
#package install
packages="openssl openvpn easy-rsa curl"
...
sndbuf 0
rcvbuf 0
push \042sndbuf 0\042
push \042rcvbuf 0\042
...
iptables -A INPUT -m state --state NEW -m $PORTL -p $PORTL --dport $PORTN -j ACCEPT
iptables -A INPUT -i tun+ -j ACCEPT
iptables -t nat -A POSTROUTING -s 10.8.0.0/24 -o eth0 -j SNAT --to-source $EIP
" > /root/iptables
...
echo "Use /etc/openvpn/newclient.sh to generate client config and run iptables file (/root/iptables)"
